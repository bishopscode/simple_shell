 ...
 */
int hsh(custom_shell_info_t *info, char **av)
{
	ssize_t r = 0;
	int builtin_ret = 0;
 ...
 * Return: void
 */
void find_cmd(custom_shell_info_t *info)
{
	char *path = NULL;
	int i, k;
	...
	path = custom_find_path(info, _getenv(info, "PATH="), info->argv[0]);
	if (path)
	{
		info->path = path;
		fork_cmd(info);
	...
	{
		if ((custom_interactive(info) || _getenv(info, "PATH=")
 ...
 * Return: void
 */
void fork_cmd(custom_shell_info_t *info)
{
	pid_t child_pid;
